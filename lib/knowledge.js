...
- **Shopping Centers**: patrulhamento com Segway i2 (áreas internas) e Segway X2 (externas/terreno irregular). Foco: cobertura e agilidade da ronda.
- **Segurança Patrimonial**: Segway i2/X2 para rondas/vigilância. Foco: mais cobertura por agente, menor tempo de resposta.
- **Logística / Last Mile**: Besouro Delivery (baú fechado, carga 1.300 kg) e Formigão G3 (compacto, difícil acesso). Foco: capacidade de carga e custo por entrega.
- **Coleta Seletiva / Limpeza Pública**: Besouro Coletor, Besouro Eco L/T e Formigão G3. Foco: acesso a vielas, ESG, economia circular.
- **Transporte de passageiros / turismo**: Joaninha G2 (até 3 passageiros).
...
# Guia rápido: qual veículo indicar
- Coleta de resíduos com contêiner/caçamba, muito volume → **Besouro Coletor** ou **Besouro Eco L/T**.
- Coleta/serviço em vielas estreitas e ladeiras, volume menor → **Formigão G3**.
- Entregas urbanas / last mile com carga em baú fechado → **Besouro Delivery**.
- Ronda/patrulhamento em piso plano ou interno (shopping, galpão) → **Segway i2**.
- Ronda em área externa / terreno irregular → **Segway X2**.
- Transporte de pessoas / turismo / eventos → **Joaninha G2**.
Confirme sempre autonomia e carga na ficha técnica (seção Produtos) antes de fechar a indicação.

# Quem procurar (encaminhe para a pessoa certa)
- Comercial, preços, descontos, propostas: **Elizabete** (Coordenadora Comercial).
- Pós-vendas, assistência técnica, garantia: **Diógenes**.
- T.I., sistemas, acessos, integrações (ERP/estoque): **Altair**.
- RH, pessoas, admissão, benefícios: **Sandra**.
- Jurídico (revisão/assinatura de contrato): setor jurídico.
- Não sabe quem? Veja **Organograma / Pessoas** na intranet.

# Objeções de vendas
Há uma **Biblioteca de Objeções** na intranet (menu Vendas) com respostas prontas (preço, concorrência, produto). Se o colaborador trouxer uma objeção, ajude com uma resposta no tom da CicloWay (produção nacional, custo até 90% menor, suporte com SLA, cases reais) e aponte a Biblioteca para mais.
