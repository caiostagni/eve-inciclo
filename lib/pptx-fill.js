...
// Quando todos os placeholders têm o mesmo nome (ex.: {{XXXX}} do Canva), substituímos
// o N-ésimo placeholder pelo values[N] (ordem: slides numéricos → parágrafos → esquerda→direita).
function fillParagraphPositional(para, values, counter) {
  const tRe = /(<a:t(?:\s[^>]*)?>)([\s\S]*?)(<\/a:t>)/g;
  const runs = []; let m;
  while ((m = tRe.exec(para))) runs.push({ open: m[1], text: m[2], close: m[3], index: m.index, len: m[0].length });
  if (!runs.length) return para;
  const full = runs.map((r) => r.text).join('');
  if (full.indexOf('{{') === -1) return para;
  const repl = (s) => s.replace(PH, () => { const v = values[counter.i]; counter.i += 1; return v != null ? xesc(v) : ''; });
  let pos = 0; const spans = [];
  for (const r of runs) { spans.push([pos, pos + r.text.length]); pos += r.text.length; }
  let split = false;
  for (const mm of full.matchAll(PH)) { const ini = mm.index, fim = mm.index + mm[0].length; if (!spans.some(([s, e]) => s <= ini && fim <= e)) { split = true; break; } }
  const newTexts = split ? runs.map((r, i) => (i === 0 ? repl(full) : '')) : runs.map((r) => repl(r.text));
  let out = '', last = 0;
  runs.forEach((r, i) => { out += para.slice(last, r.index) + r.open + newTexts[i] + r.close; last = r.index + r.len; });
  out += para.slice(last);
  return out;
...
const slideNum = (name) => { const m = name.match(/slide(\d+)\.xml$/); return m ? +m[1] : 0; };

// Preenche por posição. values = array; devolve { buf, used }.
export async function fillPptxPositional(templateBuffer, values) {
  const zip = await JSZip.loadAsync(templateBuffer);
  const counter = { i: 0 };
  const slides = Object.keys(zip.files).filter(isSlide).sort((a, b) => slideNum(a) - slideNum(b));
  for (const name of slides) {
    let xml = await zip.files[name].async('string');
    xml = xml.replace(/<a:p(?:\s[^>]*)?>[\s\S]*?<\/a:p>/g, (p) => fillParagraphPositional(p, values, counter));
    zip.file(name, xml);
  }
  const buf = await zip.generateAsync({ type: 'nodebuffer', compression: 'DEFLATE' });
  return { buf, used: counter.i };
}
