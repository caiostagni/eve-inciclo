// Cliente do RD Station MARKETING (API v2, OAuth2). Monta a "foto" do funil de LEADS.
// Fluxo: client_id + client_secret + refresh_token  ->  access_token (validade ~24h)
//        access_token  ->  segmentações  ->  contagem de contatos por etapa.
const AUTH_URL = 'https://api.rd.services/auth/token';
const BASE = 'https://api.rd.services';

// POST /auth/token — usado tanto para trocar o "code" (1ª autorização) quanto para dar refresh.
async function tokenRequest(payload) {
  const r = await fetch(AUTH_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
  if (!r.ok) {
    const body = await r.text().catch(() => '');
    const e = new Error('RD auth ' + r.status);
    e.status = r.status; e.body = body.slice(0, 300);
    throw e;
  }
  return r.json();
}

// Troca o refresh_token por um access_token de curta duração. Devolve o JSON completo
// (pode conter um refresh_token rotacionado, que o chamador deve persistir).
export async function refreshAccessToken({ clientId, clientSecret, refreshToken }) {
  const j = await tokenRequest({ client_id: clientId, client_secret: clientSecret, refresh_token: refreshToken });
  if (!j.access_token) { const e = new Error('RD auth sem access_token'); e.status = 502; throw e; }
  return j;
}

// Troca o "code" da autorização (1ª conexão) por access_token + refresh_token.
export async function exchangeCode({ clientId, clientSecret, code }) {
  const j = await tokenRequest({ client_id: clientId, client_secret: clientSecret, code });
  if (!j.refresh_token) { const e = new Error('RD auth sem refresh_token'); e.status = 502; e.body = JSON.stringify(j).slice(0, 300); throw e; }
  return j;
}

// Persistência do refresh_token (tabela integration_token).
export async function getStoredRefresh(sql) {
  try { const r = await sql`select value from integration_token where key = 'rd_mkt_refresh' limit 1`; return r[0]?.value || null; }
  catch { return null; }
}
export async function setStoredRefresh(sql, token) {
  await sql`insert into integration_token (key, value, updated_at) values ('rd_mkt_refresh', ${token}, now())
            on conflict (key) do update set value = excluded.value, updated_at = now()`;
}

async function rdGet(path, accessToken, params = {}) {
  const url = new URL(BASE + path);
  for (const [k, v] of Object.entries(params)) if (v != null) url.searchParams.set(k, v);
  const r = await fetch(url, { headers: { Authorization: 'Bearer ' + accessToken } });
  if (!r.ok) {
    const body = await r.text().catch(() => '');
    const e = new Error('RD API ' + r.status);
    e.status = r.status; e.body = body.slice(0, 300);
    throw e;
  }
  return r.json();
}

// Lista todas as segmentações da conta (paginado).
export async function listSegmentations(accessToken) {
  const out = [];
  let page = 1;
  const SIZE = 125, MAX_PAGES = 20;
  while (page <= MAX_PAGES) {
    const d = await rdGet('/platform/segmentations', accessToken, { page, page_size: SIZE });
    const arr = (d && (d.segmentations || d.items)) || [];
    out.push(...arr);
    if (arr.length < SIZE || arr.length === 0) break;
    page++;
  }
  return out;
}

// Debug: devolve a resposta crua de uma página de contatos de uma segmentação.
export async function segmentContactsPage(accessToken, id, page = 1, pageSize = 125) {
  return rdGet(`/platform/segmentations/${id}/contacts`, accessToken, { page, page_size: pageSize });
}

// Conta os contatos de uma segmentação. A API do RD não devolve total, então paginamos —
// mas em LOTES paralelos (mais rápido). Devolve { count, capped } (capped=true se bateu no teto).
async function countSegmentationContacts(accessToken, id) {
  const SIZE = 125, BATCH = 5, MAX_PAGES = 300; // teto ~37.500; além disso marca "N+"
  let total = 0, page = 1, done = false;
  while (page <= MAX_PAGES && !done) {
    const nums = [];
    for (let k = 0; k < BATCH && (page + k) <= MAX_PAGES; k++) nums.push(page + k);
    const lens = await Promise.all(nums.map((p) =>
      rdGet(`/platform/segmentations/${id}/contacts`, accessToken, { page: p, page_size: SIZE })
        .then((d) => ((d && (d.contacts || d.items)) || []).length)
        .catch(() => 0)));
    for (const len of lens) { total += len; if (len < SIZE) done = true; }
    page += BATCH;
  }
  return { count: total, capped: !done };
}

// Etapas do funil de leads da CicloWay = segmentações "(estágio no funil)" do RD Marketing.
// Mapeadas por ID (estável) com rótulos limpos. Override via env RD_MKT_FUNNEL (JSON).
// Obs.: "Visitantes" não existe como segmentação (visitantes anônimos não são contatos),
// então o funil de leads começa em "Leads".
export const DEFAULT_FUNNEL = [
  { label: 'Leads', segId: 379676 },              // "Leads (estágio no funil)"
  { label: 'Leads Qualificados', segId: 379677 }, // "Leads Qualificados (estágio no funil)"
  { label: 'Oportunidades', segId: 379679 },      // "Oportunidades"
  { label: 'Vendas', segId: 379678 },             // "Clientes (estágio no funil)"
];

// Retorna { kind:'leads', stages:[{name,count,value:0}], totalCount, totalValue:0 }.
// stages: array de { label, segId }. Se omitido, usa DEFAULT_FUNNEL.
export async function fetchLeadFunnel(accessToken, stages = DEFAULT_FUNNEL) {
  const out = [];
  let totalCount = 0, anyCapped = false;
  for (const st of stages) {
    const { count, capped } = await countSegmentationContacts(accessToken, st.segId);
    out.push({ name: st.label, count, value: 0, capped });
    totalCount += count; if (capped) anyCapped = true;
  }
  return { kind: 'leads', stages: out, totalCount, totalValue: 0, capped: anyCapped };
}
