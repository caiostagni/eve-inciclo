...
export async function fetchLeadFunnel(accessToken, stages = DEFAULT_FUNNEL) {
  ...
  for (const st of stages) {
    const count = await countSegmentationContacts(accessToken, st.segId);
    out.push({ name: st.label, count, value: 0 });
    totalCount += count;
  }
  return { kind: 'leads', stages: out, totalCount, totalValue: 0 };
}
