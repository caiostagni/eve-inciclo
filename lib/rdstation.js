...
export async function listSegmentations(accessToken) {
  const out = [];
  let page = 1;
...
// Debug: devolve a resposta crua de uma página de contatos de uma segmentação.
export async function segmentContactsPage(accessToken, id, page = 1, pageSize = 125) {
  return rdGet(`/platform/segmentations/${id}/contacts`, accessToken, { page, page_size: pageSize });
}

const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

// Busca 1 página; em rate limit (429) espera e tenta de novo (1x). Outros erros: propaga.
async function pageWithRetry(accessToken, id, page, size) {
  try {
    return await rdGet(`/platform/segmentations/${id}/contacts`, accessToken, { page, page_size: size });
  } catch (e) {
    if (e.status === 429) { await sleep(2500); return rdGet(`/platform/segmentations/${id}/contacts`, accessToken, { page, page_size: size }); }
    throw e;
  }
}

// Conta os contatos de uma segmentação. O RD limita page_size a 125 e não devolve total,
// então paginamos SEQUENCIALMENTE (rate-limit safe). Erro de verdade PROPAGA (nunca grava
// contagem errada). Devolve { count, capped } — capped=true se bateu no teto (~10.000).
async function countSegmentationContacts(accessToken, id) {
  const SIZE = 125, MAX_PAGES = 80; // teto ~10.000; acima disso o rótulo vira "10.000+"
  let total = 0, capped = true;
  for (let page = 1; page <= MAX_PAGES; page++) {
    const d = await pageWithRetry(accessToken, id, page, SIZE);
    const arr = (d && (d.contacts || d.items)) || [];
    total += arr.length;
    if (arr.length < SIZE) { capped = false; break; }
  }
  return { count: total, capped };
}

// Etapas do funil de leads da CicloWay = segmentações "(estágio no funil)" do RD Marketing.
// Mapeadas por ID (estável) com rótulos limpos. Override via env RD_MKT_FUNNEL (JSON).
// Obs.: "Visitantes" não existe como segmentação (visitantes anônimos não são contatos),
// então o funil de leads começa em "Leads".
export const DEFAULT_FUNNEL = [
  { label: 'Leads', segId: 379676 },              // "Leads (estágio no funil)"
  { label: 'Leads Qualificados', segId: 379677 }, // "Leads Qualificados (estágio no funil)"
  { label: 'Oportunidades', segId: 379679 },      // "Oportunidades"
  { label: 'Vendas', segId: 379678 },             // "Clientes (estágio no funil)"
];

// Retorna { kind:'leads', stages:[{name,count,value:0}], totalCount, totalValue:0 }.
// stages: array de { label, segId }. Se omitido, usa DEFAULT_FUNNEL.
export async function fetchLeadFunnel(accessToken, stages = DEFAULT_FUNNEL) {
  const out = [];
  let totalCount = 0, anyCapped = false;
  for (const st of stages) {
    const { count, capped } = await countSegmentationContacts(accessToken, st.segId);
    out.push({ name: st.label, count, value: 0, capped });
    totalCount += count; if (capped) anyCapped = true;
  }
  return { kind: 'leads', stages: out, totalCount, totalValue: 0, capped: anyCapped };
}
