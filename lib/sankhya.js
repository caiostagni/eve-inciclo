...
// Padrão do gateway Sankhya: usuário/senha vão nos HEADERS (não no body).
export async function getBearer(cfg) {
  const r = await fetch(cfg.authUrl || DEFAULT_AUTH_URL, {
    method: 'POST',
    headers: { appkey: cfg.appkey, token: cfg.token, username: cfg.username, password: cfg.password },
  });
  ...
  if (!r.ok || j.error || !bearer) {
    ...
    e.status = r.ok ? 502 : r.status; e.body = raw.slice(0, 400);
    throw e;
  }
  return bearer;
}
